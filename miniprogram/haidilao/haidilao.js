...
// 可选代理出口：IPv6-only VPS 借助 WARP/代理访问仅 IPv4 的接口（仅 Node 生效）
// 设置环境变量 WARP_PROXY，例如 socks5://127.0.0.1:40000 或 http://127.0.0.1:xxxx
let PROXY_AGENT = null;
if ($.isNode() && process.env.WARP_PROXY) {
    try {
        const _p = process.env.WARP_PROXY.trim();
        let _agent;
        if (/^socks/i.test(_p)) {
            const { SocksProxyAgent } = require('socks-proxy-agent');
            _agent = new SocksProxyAgent(_p);
        } else {
            const { HttpsProxyAgent } = require('hpagent');
            _agent = new HttpsProxyAgent({ proxy: _p });
        }
        PROXY_AGENT = { http: _agent, https: _agent };
        console.log(`[INFO] 已启用代理出口: ${_p}`);
    } catch (e) {
        console.log(`[WARN] 代理初始化失败(${e.message})，将尝试直连`);
    }
}
